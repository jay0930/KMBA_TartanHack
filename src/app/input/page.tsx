...
function CalendarStep({ onNext, userId }: { onNext: (events: CalendarEvent[]) => void; userId: string }) {
  ...
  const handleAddEvent = async () => {
    ...
    const newEvent: CalendarEvent = { time, title: cleanTitle, location, emoji: '⏳' };
    ...
    try {
      const res = await backendFetch('/api/emoji/assign', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ titles: [cleanTitle] }),
      });
      const data = await res.json();
      const emoji = data.emojis?.[0] || '📌';
      setEvents(prev => prev.map((e, i) => i === newIdx ? { ...e, emoji } : e));
    } catch {
      setEvents(prev => prev.map((e, i) => i === newIdx ? { ...e, emoji: getEmojiForEvent(cleanTitle) } : e));
    }
  };
...
function PhotoStep({ onNext }: { onNext: (photos: PhotoEvent[]) => void }) {
  ...
  const [captions, setCaptions] = useState<{ emoji: string; caption: string; time: string; time_source?: string }[]>([]);
